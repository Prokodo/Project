...
const TenantsPopupForm = ({ isAdmin } : { isAdmin: boolean }) => {
    ...
    return (
        <div className="mx-4 mt-2">
            <button onClick={togglePopup}
                ...
                {isAdmin ? "Add new user" : "Add new tenant"}
            </button>
            ...
            {isOpen && (
                <div className="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-white rounded-lg shadow-lg w-full max-w-2xl p-6 relative">
                        ...
                        <h2 className="text-2xl font-bold text-center mb-6">{isAdmin? "Add new user": "Add new tenant"}</h2>
                        <TenantsRegistrationForm setIsOpen={setIsOpen} isAdmin={isAdmin} />
                    </div>
                </div>
            )}
        </div>
